...
Menu (solo / host / join, language, mode, continue) · lobby · intro comic + mission brief · HUD (bars, inventory, objective line; the key legend is always on screen; [I] opens the "HOW" panel **and** the objective arrow / "search the rooms" hint — hidden until asked, so players explore first; threat ring, multi-row prompt with colored key badges) · [Tab] plan · pause · CCTV (archive log advances on [SPACE]; friends drawn with names, unnamed survivors as blue dots) · notes · chapter comics · death/respawn · ending cutscene (ladder pickup, lift-off, eyes closing) · winner screen.

**How they find you.** Every floor is stamped into a 0.5 m walkable grid (walls and furniture block; a SHUT doorway does not, because a hunting zombie forces it open in about a second — but an OPEN door leaf is solid, and the grid is rebuilt whenever a door swings). A breadth-first flow field from the hunted human gives each cell a step count, and a zombie simply walks to the neighbouring cell with the smaller number — so it rounds partitions, leaves rooms through the doorway and follows you down the corridor instead of grinding along a wall. Line of sight still decides whether it has *seen* you; the grid decides where it walks — and a clear line of sight is not enough to charge straight in, the straight line has to be walkable on the grid too (sight passes over a desk, a body does not). If the grid ever fails (the roof, a spot outside it) it falls back to a doorway heuristic, then to straight-line steering.

**What they cannot do.** `reachClear()` is a separate test from `losClear()`: sight passes through a doorway, an arm does not pass through a wall or a shut door, so nothing can bite you from the other side of the plaster — the old "something is hitting me and I cannot see it" bug. Anything whose body and both shoulders are out of sight is not drawn at all, so a swinging arm can no longer poke through thin plaster and give away where something is waiting. Bodies are 0.46 m wide for collision — wide enough that shoulders and swinging arms stay out of the walls, narrow enough to still fit between the desks; walking into one shoves it aside (a brute or the boss barely moves), so a zombie standing in a doorway is an obstacle, not a locked door.
